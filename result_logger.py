...
class ResultLogger:
        ...
        def print_results(results):
            ...
            for (strategy_name, sma_period), (final_value, num_trades, win_rate) in results.items():
                print(f'Strategy: {strategy_name}, SMA Period: {sma_period}, Final Portfolio Value: {final_value:.2f}, Number of Trades: {num_trades}, Win rate: {win_rate}')
        ...
        def plot_results(results, filename='results.png',  horizontal_line=None):
            ...
            # Create a bar plot
            ax1.bar(x, final_values, tick_label=strategy_names)
            plt.yticks(range(0, round(max(final_values))+500, 100))  # Set the ticks on the y-axis
            plt.ylim(max(final_values)-1000, max(final_values)+1000)  # Set the limits of the y-axis


             # Create a second y-axis for the win rates
            ax2 = ax1.twinx()
            ax2.plot(x, win_rates, color='g', marker='o')
            
            # Set the ticks and limits for the right y-axis
            ax2.set_yticks(range(0, round(max(win_rates))+1, 1))  # Set the ticks on the right y-axis
            ax2.set_ylim(max(win_rates)-5, max(win_rates)+5)  # Set the limits of the right y-axis
